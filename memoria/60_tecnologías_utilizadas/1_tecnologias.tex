...
\begin{itemize}
    ...
    \item \textbf{react-native-maps} y \textbf{expo-location}: librerías utilizadas para la integración del mapa y el acceso a la geolocalización del dispositivo, respectivamente.
...
\begin{itemize}
    \item \textbf{Firebase Storage} \cite{firebasestorage}: espacio de almacenamiento en la nube donde se guardan las imágenes de los pasos de cada cofradía. Se ha mantenido este servicio, en lugar de gestionar el almacenamiento de ficheros desde el backend propio, porque resuelve de forma nativa aspectos como la compresión, el servido eficiente de imágenes o el control de acceso a los ficheros, sin aportar valor académico adicional programarlo desde cero.
    \item \textbf{Firebase Cloud Messaging (FCM)} \cite{firebasefcm}: servicio de notificaciones \emph{push}. El backend propio, a través del \textbf{Firebase Admin SDK} \cite{fcmadminsdk}, es quien decide cuándo se envía una notificación (por ejemplo, cuando una Junta de Cofradías publica una alerta) y delega en FCM la entrega al dispositivo del usuario.

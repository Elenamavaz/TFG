...
\begin{itemize}
    \item \texttt{pasos} y \texttt{puntos\_ruta} son tablas independientes con una clave foránea a su tabla ``padre'' (\texttt{cofradia\_id} y \texttt{recorrido\_id}, respectivamente), en lugar de subcolecciones anidadas como en el planteamiento inicial con Firestore. Al tratarse de un modelo relacional, no es necesario un mecanismo especial para expresar que ``siempre se consultan junto a su padre'': basta con una consulta que una (\emph{join}) ambas tablas por esa clave foránea.
    \item \texttt{posicion\_actual} se modela como una tabla independiente, con \texttt{procesion\_id} como clave primaria y clave foránea a la vez (relación 1:1), en lugar de como columnas dentro de la propia tabla \texttt{procesiones}. El motivo es el mismo que en el planteamiento inicial: estas escrituras frecuentes de posición (cada 30 segundos) no deben afectar a los datos más estables de la procesión (horario, recorrido, pasos), que rara vez cambian; mantenerlas en una tabla aparte evita bloquear en cada escritura una fila con muchas más columnas de las necesarias. Cada actualización se realiza como una operación \texttt{UPSERT}\footnote{UPSERT: operación que inserta una fila si no existe todavía, o la actualiza si ya existe, en una sola sentencia (en PostgreSQL, \texttt{INSERT ... ON CONFLICT DO UPDATE}).} que sobrescribe la fila existente con la posición agregada de los cofrades de esa procesión, de forma análoga al documento único que se sobrescribía en el planteamiento con Firestore. La aplicación cliente consulta esta posición mediante la API propia (\texttt{GET~/procesiones/\{id\}/ubicacion}) con la cadencia que exige el requisito RNF-08, tal y como se explica en el apartado \nameref{sec:decisionesTecnologicas}.
    ...
    \item La tabla \texttt{usuarios} solo contiene los perfiles de rol elevado (cofrade, junta de cofradías, administrador): el ciudadano no se registra, por lo que no genera ninguna fila en esta tabla para poder usar la aplicación. Los tres roles no se autentican de la misma forma, tal y como recoge el modelo de dominio (Figura~\ref{fig:DiagramDomain}): la junta de cofradías y el administrador sí tienen credenciales propias (\texttt{email} y una contraseña almacenada como \emph{hash}\footnote{Hash: transformación irreversible de una contraseña en una cadena de caracteres, de forma que el sistema pueda comprobar que una contraseña es correcta sin necesidad de guardarla en texto plano.} mediante BCrypt, el algoritmo utilizado por defecto por Spring Security \cite{springsecurity}), mientras que el cofrade \textbf{no tiene contraseña}: obtiene su rol únicamente al introducir un \textbf{código de acceso} válido, gestionado en la tabla \texttt{codigos\_acceso} (con estados \texttt{EMITIDO}, \texttt{VALIDADO} y \texttt{REVOCADO}), tal y como recoge el caso de uso ``Acceso como cofrade'' (Sección~\ref{cap:modeloInteraccion}). Al validar el código, la API crea (si no existía ya) su fila en \texttt{usuarios} --con \texttt{email} y \texttt{password\_hash} nulos-- y devuelve directamente un token JWT, sin que en ningún momento se le pida establecer una contraseña: el propio código de acceso actúa como credencial. En los tres casos, el rol queda guardado como columna y se comprueba en la capa de servicio de la API (\emph{Service}, Sección~\ref{cap:1_iteracion}) antes de autorizar cualquier operación protegida, cubriendo así el requisito RNF-05. Cada código de acceso lleva además una \textbf{referencia a la cofradía} que lo emite, de forma que un cofrade que valida su código queda automáticamente vinculado a esa cofradía y puede compartir su ubicación en tiempo real durante las procesiones en las que participa, que es la única acción que el modelo de dominio le permite realizar directamente sobre el sistema (Figura~\ref{fig:DiagramDomain}).
    \item Las relaciones N:M del modelo de dominio --pasos que desfilan en varias procesiones, y notificaciones que se entregan a varios usuarios-- se resuelven mediante \textbf{tablas intermedias} (\texttt{pasos\_procesiones} y \texttt{notificaciones\_entregadas}, esta última con los atributos adicionales \texttt{leida} y \texttt{fecha\_lectura}), la forma habitual de modelar relaciones muchos-a-muchos en un esquema relacional.
\end{itemize}

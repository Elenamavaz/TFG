...
La comunicación con los usuarios se modela mediante una única clase \texttt{Notificacion} --ya no hay una jerarquía \texttt{Aviso}/\texttt{Alerta} como se planteó inicialmente: al revisar qué tipos se usaban de verdad se vio que el cliente decide el color con el que mostrar cada notificación solo a partir de su prioridad, nunca de una distinción más fina entre subtipos, así que esa jerarquía no se estaba ganando su sitio frente a la complejidad de mantenerla--. Cada \texttt{Notificacion} pertenece a una \texttt{Ciudad} y tiene un tipo (\texttt{INICIO}, \texttt{FIN}, \texttt{INCIDENCIA}, \texttt{CAMBIO\_HORARIO} o \texttt{CANCELACION}: los dos primeros los genera el propio sistema al cambiar el estado de un \texttt{Evento}, el resto los crea la Junta de Cofradías a mano) y, salvo en \texttt{INICIO}/\texttt{FIN}, una prioridad (\texttt{BAJA}, \texttt{MEDIA} o \texttt{ALTA}). Sin \texttt{EntregaNotificacion}: no se registra si un usuario concreto la ha leído o no --el ciudadano no tiene cuenta con la que asociar esa lectura--, simplemente se consulta la lista de notificaciones vigentes de la ciudad. Además de poder consultarse dentro de la aplicación, cada \texttt{Notificacion} se reenvía como notificación \emph{push} a los dispositivos registrados en su ciudad (clase \texttt{DispositivoPush}, un token de Expo Push asociado a una \texttt{Ciudad}, sin ningún \texttt{Usuario} al que pertenecer).
...
% TODO: diagramas de secuencia pendientes de añadir - Editar ciudad, Dar de baja ciudad y Crear notificación.
